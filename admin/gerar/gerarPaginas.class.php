<?php
    ...
    while($row_table = mysql_fetch_array($sql_table)):
        ...
        $sql_coluna2 = mysql_query("SHOW COLUMNS FROM $tabela WHERE Field <> '$campo_id'");
        echo "\n\t".'$this->listar = array(\'id\',';
        while($row_coluna2 = mysql_fetch_array($sql_coluna2)):
        ...
        while($row_coluna = mysql_fetch_array($sql_coluna)):
            if( like('%texto%', $row_coluna['Field']) || like('%descricao%', $row_coluna['Field'])){
            ...
            }elseif(like('%imagem%', $row_coluna['Field']) || like('%texto%', $row_coluna['banner']) || like('%icone%', $row_coluna['Field']) || like('%texto%', $row_coluna['foto']) || like('%thumb%', $row_coluna['Field'])){
                $campos = $campos."\n\t\t\"".$row_coluna['Field']."\" => array('".ucwords(strtolower(str_replace('_',' ',$row_coluna['Field'])))."','upload-img','../assets/images/',false,false),";
            }else{
