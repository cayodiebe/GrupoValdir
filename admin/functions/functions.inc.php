<?php
	class Funcoes {
		...
		public function gerarMenu($paginas,$url_painel){
			...
			if(isset($_GET['m']))
            	$menu_pronto .= '<li><a href="'.$url_painel.'" class="open"><i class="entypo-home"></i><span>Início</span></a></li>';
            else
            	$menu_pronto .= '<li class="opened active"><a href="'.$url_painel.'" class="open"><i class="entypo-home"></i><span>Início</span></a></li>';
            foreach($paginas->paginas_menu as $pagina):
            	if(is_array($pagina)):
            		...
            		$menu_pronto .= '<li class="'.$class.'"><a href="#">'.$pagina_atual->icone.'<span>'.$pagina[0].'</span></a>';
            		$menu_pronto .= '<ul>';
            		foreach ($pagina as $pagina_submenu):
            			if(is_array($pagina_submenu)):
			            	$menu_pronto .= '<li><a href="#">'.$pagina_atual->icone.'<span>'.$pagina_submenu[0].'</span></a>';
					        $menu_pronto .= '<ul>';
        					foreach ($pagina_submenu as $pagina_sub_submenu):
	            				if($pagina_sub_submenu <> $pagina_submenu[0]):
					            	...
					            	$menu_pronto .= '<li><a href="#">'.$pagina_atual->icone.'<span>'.$pagina_atual->titulo.'</span></a>';
					        		$menu_pronto .= '<ul>';
	            	...
	            	$menu_pronto .= '<li class="'.$class.'"><a href="#">'.$pagina_atual->icone.'<span>'.$pagina_atual->titulo.'</span></a>';
	                if(isset($pagina_atual->operacoes)):
	                    $menu_pronto .= '<ul>';
	                    foreach($pagina_atual->operacoes as $operacao_atual):
	                    	if($operacao_atual == 'listar') $icone = '<i class="entypo-list"></i>';
						...
						switch($campos_b):
							case 'acao':
								foreach ($campo_b as $campos_a => $campo_a):
									if($campo_a == 'editar'):
										$tabela_gerada .= '<a href="?m='.$modulo.'&t=editar&id='.$row[$campoID].'" title="Editar" class="btn_acoes btn btn-blue btn-sm btn-icon icon-left"><i class="entypo-pencil"></i>Editar</a>';
									elseif($campo_a == 'deletar'):
										$tabela_gerada .= '<a href="?m='.$modulo.'&t=deletar&id='.$row[$campoID].'" title="Deletar" class="btn_acoes btn btn-red btn-sm btn-icon icon-left"><i class="entypo-cancel"></i>Deletar</a>';
									elseif($campo_a == 'mail'):
		...
		public function gerarCampos($campos,$campo,$db){
			...
			switch($campo[1]):
				...
				break;
				case 'input-block':
					$retorno .= '<input '.$dica.$sufixo.' class="form-control" name="'.$campos.'" maxlength="'.$campo[2].'" placeholder="'.$campo[3].'" value="'.$db[$campos].'" type="'.$campo[4].'" readonly>';
				break;
				case 'input-preco':
				...
				break;
				case 'textarea':
					$retorno .= '<textarea '.$dica.$sufixo.' class="form-control" name="'.$campos.'" maxlength="'.$campo[2].'" placeholder="'.$campo[3].'">'.$db[$campos].'</textarea>';
				break;
				case 'textarea2':
					$retorno .= '<textarea '.$dica.$sufixo.' class="form-control" name="'.$campos.'" maxlength="'.$campo[2].'" placeholder="'.$campo[3].'">'.$db[$campos].'</textarea>';
				break;
				...
				case 'upload-file':
					if($db[$campos] == '' || isset($_GET[$campos])){
						...
						$retorno .= 'Arquivo: <a href="../assets/uploads/'.$nome_arquivo.'" target="_blank"><strong>'.$nome_arquivo.'</strong></a>';
						$retorno .= '<a href="?'.$_SERVER['QUERY_STRING'].'&'.$campos.'=false" style="margin-left:5px;">Remover</a>';
					}
				break;
				case 'checkbox':
				...
				case 'datetime':
					...
					<div class="date-and-time">
						<input '.$dica.$sufixo.' type="text" class="form-control datepicker" data-format="D, dd MM yyyy" name="'.$campos.'">
						<input '.$dica.$sufixo.' type="text" class="form-control timepicker" data-template="dropdown" data-show-seconds="true" data-show-meridian="true" data-minute-step="5" data-second-step="5" name="'.$campos.'2">
					</div>
					';
					// $retorno .= '<input '.$dica.$sufixo.' class="form-control timepicker" data-format="D, dd MM yyyy" name="'.$campos.'" placeholder="'.$campo[2].'"  type="text">';
				break;
			endswitch;
			$retorno .= '</div>';
			$retorno .= '</div>';
		...
		public function gerarBotoes($acao){
			$botoes = '';
			$botoes .= '<div class="form-group"><div class="col-sm-offset-2 col-sm-10" style="text-align:right;">';
			switch($acao):
				case 'inserir':
