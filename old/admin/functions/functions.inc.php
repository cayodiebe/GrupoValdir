	...
	class Funcoes {
							...
							case 'acao':
								foreach ($campo_b as $campos_a => $campo_a):
									...
									elseif($campo_a == 'deletar'):
										$tabela_gerada .= '<a style="margin-left:5px;" href="?m='.$modulo.'&t=deletar&id='.$row[$campoID].'" title="Deletar" class="btn btn-red btn-sm btn-icon icon-left"><i class="entypo-cancel"></i>Deletar</a>';
									elseif($campo_a == 'mail'):
		...
		public function gerarCampos($campos,$campo,$db){
			...
			switch($campo[1]):
				...
				break;
				case 'link-user':
					$retorno .= '<a target="_blank" href="?m=cadastro_pf&t=editar&id='.$db[$campos].'" title="">Visualizar Dados</a><br>';
					break;
				case 'link':
					$aux = explode(',', $db[$campos]);
					foreach ($aux as $value) {
						if( intval($value) > 0 ){
							$sql = mysql_query("SELECT * FROM carrinho WHERE id = '".$value."' LIMIT 1 ");
							$row = mysql_fetch_array($sql);
							if( $row['tipo'] == 'produto' ){
								$sql = mysql_query("SELECT * FROM produtos WHERE id = '".$row['item']."' LIMIT 1 ");
								$row = mysql_fetch_array($sql);
								$retorno .= '<a target="_blank" href="?m=carrinho&t=editar&id='.$value.'" title="">'.$row['nome'].'</a><br>';
							}
							else{
								$sql = mysql_query("SELECT * FROM compre_junto WHERE id = '".$row['item']."' LIMIT 1 ");
								$row = mysql_fetch_array($sql);
								$retorno .= '<a target="_blank" href="?m=compre_junto&t=editar&id='.$row['id'].'" title="">'.$row['nome'].'</a><br>';
							}
						}
					}
					break;
				...
				case 'select':				
					if($campo[5]):
						$retorno .= '<select '.$dica.$sufixo.' id="my-select" multiple="multiple" name="'.$campos.'[]" class="form-control multi-select">';
					else:
		...
		public function uploadImagem($arquivo,$pasta,$width,$height){
			...
			if( isset($arquivo["tmp_name"]) && $arquivo["tmp_name"] != '' ){
				require_once("../assets/wideimage/WideImage.php");
			...
			if($ext[1]=='png'):
				$image->saveToFile($caminho_imagem,6);
			else:
				$image->saveToFile($caminho_imagem,70);
			endif;
		...
		public function salvarDados($campos,$dados,$arquivos,$id,$db,$acao,$banco,$tabela){
			...
			if(!empty($objeto->valor_campos)):
				switch($acao):
					case 'editar':
					echo $base_site;
						$aux = $banco->update($objeto);
						foreach($objeto->valor_campos as $key => $value):
						    if($key == 'status' && $aux == 1 ){
						    	$sql = mysql_query("SELECT * FROM pedidos WHERE id = '".$objeto->valorID."' LIMIT 1 ");
								$row = mysql_fetch_array($sql);
								$id_user = $row['id_user'];
								$id_pedido = $row['id'];
						    	$url = 'http://toocano.com/projetos/sylvaz/enviarEmailAlteracaoStatus';
							    $data = array(  
							                    'status' => $value,
							                    'id_user' => $id_user,
							                    'id_pedido' => $id_pedido);
